...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `mysp_thongkePTTheoDiachi`(IN _loai int, IN _limit int)
BEGIN
	if _loai = 1 then
		select GET_DUONG(diachi) as value, count(*) as counter 
		from phongtro 
		group by GET_DUONG(diachi)
		order by counter desc
		limit _limit;
    else
		select SPLIT_STR(diachi,',', _loai) as value, count(*) as counter 
		from phongtro 
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `mysp_thongkePTTheoInput`(IN _column varchar(45), IN _limit int)
BEGIN
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `mysp_thongkePTTheoTienVaDientich`(IN _column varchar(45), IN _limit int)
BEGIN
...
-- Dump completed on 2016-12-04 23:57:29
